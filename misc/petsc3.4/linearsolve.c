...
int main(int argc,char **args)
{
  ...
  PetscInt n, its;
  PetscBool flg;
  PetscBool useMUMPS = PETSC_FALSE;
  PetscBool useSuperLU = PETSC_FALSE;
...
/* RHS     */
  ...
  if (!flg) SETERRQ(PETSC_COMM_WORLD,1,"Must indicate binary RHS file with the -r option");
  ...
  ierr = VecLoad(b,fdrhs);CHKERRQ(ierr);
...
/* Read matrix name */
  ...
  if (!flg) SETERRQ(PETSC_COMM_WORLD,1,"Must indicate binary matrix file with the -m option");
...
/* Flag for direct solver */
  ...
  if ((useMUMPS) && (useSuperLU)) {
    SETERRQ(PETSC_COMM_WORLD,1,"Cannot use both MUMPS and SuperLU!");
  }
...
/* Output file */
  ...
  if (!flg) SETERRQ(PETSC_COMM_WORLD,1,"Must indicate binary output file with the -o option");
