...
/* Matrices */
  ...
  if (!flg1) SETERRQ(1,"Must indicate binary matrix file with the -me option");
...
/*  Create/read Ae and Ai here to generate layout information */
    if ((useProfiles) && (numProcessors>1)) {
      ierr = MatCreate(PETSC_COMM_WORLD,&Ae);CHKERRQ(ierr);
      ierr = MatSetSizes(Ae,lSize,lSize,PETSC_DECIDE,PETSC_DECIDE);CHKERRQ(ierr);
      ierr = MatSetFromOptions(Ae);CHKERRQ(ierr);
	  strcpy(tmpFile,"");
	  sprintf(tmpFile,"%s%02d",mateFile,0);
      ierr = MatLoadIntoMatrix2(tmpFile,Ae);CHKERRQ(ierr);
	  ...
	  ierr = MatLoad(fd,MATMPIAIJ,&Ae);CHKERRQ(ierr);
    ...
    if ((useProfiles) && (numProcessors>1)) {
      ...
      ierr = MatLoadIntoMatrix2(mateFile,Ae);CHKERRQ(ierr);
...
/* Initial condition     */
  ...
  if (pickupFromFile) {
    ...
    for (itr=0; itr<numTracers; itr++) {
      ierr = VecLoadIntoVector(fdp,v[itr]);CHKERRQ(ierr);
    }
    ...
    if (flg1) {  /* read from file */
      ...
      for (itr=0; itr<numTracers; itr++) {
        ...
        ierr = VecLoadIntoVector(fd,v[itr]);CHKERRQ(ierr);
        ierr = PetscViewerDestroy(fd);CHKERRQ(ierr);          
      }  
    } else {  /* set to zero */
...
/* reread initial conditions (these were read once already above, but do it again since its easier to code) */    
        ierr = PetscPrintf(PETSC_COMM_WORLD,"Tracer %d: reading initial condition from file %s\n", itr,iniFile[itr]);CHKERRQ(ierr);
        ierr = VecLoadIntoVector(fdin[itr],v[itr]);CHKERRQ(ierr);
      } 
      if (useExternalForcing) ierr = reInitializeExternalForcing(tc,Iterc,numTracers,v,uef);CHKERRQ(ierr);          
    }
#endif
...
/* write final pickup */  
  ...
  ierr = VecDestroyVecs(v,numTracers);CHKERRQ(ierr);  
