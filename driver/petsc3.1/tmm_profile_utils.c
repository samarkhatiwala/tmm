...
/*   } else { */
/*     SETERRQ(PETSC_ERR_ARG_OUTOFRANGE,"Unknown type"); */
/*   } */
/*  */
/*   ierr = PetscMalloc(m1,&tmpArr);CHKERRQ(ierr); */
/*   ierr = PetscViewerBinaryOpen(PETSC_COMM_SELF,fileName,FILE_MODE_READ,&fd);CHKERRQ(ierr); */
...
/*   static PetscInt iCurrTimeReadLast=-1; */
  ...
  if (user->firstTime) {
    if (numPeriods>MAX_FORCING_PERIODS) {
      SETERRQ(1,"Number of allowable arrays in PeriodicArray struct exceeded by requested number ! Increase MAX_FORCING_PERIODS.");
    }      
