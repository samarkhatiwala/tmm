...
if 'PETSC_DIR' in os.environ:
  ...
  raise RuntimeError("You must set the PETSC_DIR environment variable before building tmm4py")

if 'PETSC_ARCH' in os.environ:
  petsc_arch = os.environ['PETSC_ARCH']
else:
  raise RuntimeError("You must set the PETSC_ARCH environment variable before building tmm4py")
...
if 'TMM_DIR' in os.environ:
  ...
  raise RuntimeError("You must set the TMM_DIR environment variable before building tmm4py")
...
setup(name="TMMInterface", ext_modules=cythonize(extensions,language_level="3",include_path=include_paths))
