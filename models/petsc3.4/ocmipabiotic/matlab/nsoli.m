...
%
% SPK 11/24/07
global firstTime startNewtonCorrection startLineSearch
global iterCounter
iterCounter=0;
%
...
%
    startNewtonCorrection=1;
    rat = fnrm/fnrmo;
    fnrmo = fnrm; 
    itc = itc+1;
    disp('starting krylov search')    
...
%
    ...
    disp(['lambda: ' num2str(lambda) ' ,Norm at x: ' num2str(nf0) ' ,Norm at xt: ' num2str(nft)])    
...
%
        ...
        ffc = nft*nft;
        iarm = iarm+1;
        disp(['new lambda: ' num2str(lambda) ' ,Norm at xt: ' num2str(nft)])    
...
%
    ...
    disp(['final lambda: ' num2str(lambda) ' ,residual norm: ' sprintf('%15.10g',fnrm)])
...
% GMRES iteration
%
disp([num2str(0) ' GMRES residual norm: ' sprintf('%15.10g',rho) ', errtol: ' sprintf('%15.10g',errtol)])
...
%
    rho = abs(g(k+1));
    disp([num2str(k) ' GMRES residual norm: ' sprintf('%15.10g',rho) ', errtol: ' sprintf('%15.10g',errtol)])
    error = [error,rho];
%
