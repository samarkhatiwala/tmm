...
#     /*Data for diagnostics */
        ...
        if p.useCarbon:
          ef.localco2airseafluxdiag=np.zeros(lNumProfiles)
...
# /* Interpolate atmospheric pCO2   */
      ...
      for ip in range(lNumProfiles):
        ...
        if p.useCarbon:
          if not p.useVirtualFlux: # /* use the local surface value to calculate E-P contribution */
            DICemp=state.c[idic][kl]
            ALKemp=state.c[ialk][kl]

        bgc_tracer = [state.c[itr][kl:kl+nzloc] for itr in range(numTracers)]
        bgc_tracer = List(bgc_tracer)
 
# C     Copy physics and geometry
        bgc_kloc=nzloc
        bgc_swr=p.localswrad[ip]
        bgc_tau=p.localtau[ip]
        bgc_seaice=p.localfice[ip]
        bgc_wind=p.localwind[ip]
        bgc_atmosp=p.localatmosp[ip]
        bgc_globalrunoff = ef.GRunoff

        bgc_dz=p.localdz[kl:kl+nzloc]
        bgc_theta=p.localTs[kl:kl+nzloc]
        bgc_salt=p.localSs[kl:kl+nzloc]
        bgc_fedep=p.localFes[kl:kl+nzloc]
        bgc_runoffvol=p.localrunoffvol[kl:kl+nzloc]
...
# C Assign the tracers in "wet" grid boxes
        ...
        if doDiagnostics:
          localfbgc=[ef.localfbgc[i][kl:kl+nzloc] for i in range(ef.numDiag)]
          localfbgc=List(localfbgc)

        bgc_geomr=List([bgc_dz,wdet,fcaco3])
        bgc_forcingsurf=List([bgc_swr,bgc_tau,bgc_seaice,bgc_wind,bgc_atmosp,bgc_globalrunoff,emp])
        bgc_forcingprof=List([bgc_theta,bgc_salt,bgc_fedep,bgc_runoffvol])
        
        sph, co2airseaflux, flux_bury = BGC_MODEL(p.useCarbon,p.useRunoff,p.bgc_dt,p.bgc_timesteps,bgc_tracer,bgc_kloc,p.bgc_keuph,bgc_geomr, \
                                                  bgc_forcingsurf,bgc_forcingprof,sph,pco2atm,dicgave,alkgave,localfbgc)

# C     Send data back
        if p.useCarbon:
          ef.localph[ip]=sph
          localco2airseaflux=co2airseaflux

        localburial=flux_bury
          
        if p.useCarbon:
          if (ef.useAtmModel):
            localFocean = localFocean + (localco2airseaflux/p.DeltaT)*p.localdA[ip]*(12.0/1.e18)*secondsPerYear #/* PgC/y */

# /* integrate burial in sediment over area and all profiles on each processor */
        ef.localFburial = ef.localFburial + localburial*p.localdA[ip]
...
#   } /* end loop over profiles */
      ...
      if p.useCarbon:
        if (ef.useAtmModel):
          ef.Focean=TMM.globalScalarSum(localFocean)
      ...
      if (ef.calcDiagnostics):
        if (Iter0+iLoop>=(ef.diagTimer.startTimeStep)):  #/* start time averaging (note: startTimeStep is ABSOLUTE time step) */  
          ...
          if (ef.diagTimer.count<=ef.diagTimer.numTimeSteps): #/* still within same averaging block so accumulate */
            ...
            if p.useCarbon:
              ef.localco2airseafluxdiagavg[:]=ef.localco2airseafluxdiag[:]+ef.localco2airseafluxdiagavg[:]
...
@njit(cache=False)
def BGC_MODEL(useCarbon, useRunoff, bgc_dt, bgc_timesteps, bgc_tracer, bgc_kloc, bgc_keuph, bgc_geomr, \
              bgc_forcingsurf, bgc_forcingprof, sph,pco2atm,dicgave,alkgave,localfbgc):

  bgc_dz,wdet,fcaco3=bgc_geomr
  bgc_swr,bgc_tau,bgc_seaice,bgc_wind,bgc_atmosp,bgc_globalrunoff,emp=bgc_forcingsurf
  bgc_theta,bgc_salt,bgc_fedep,bgc_runoffvol=bgc_forcingprof
  
# ! Depth of the euphotic zone
  bgc_keuphloc = np.fmin(bgc_kloc,bgc_keuph)
  
  doDiagnostics=(localfbgc is not None)
...
#       ENDDO !internal time loop
  ...
  return sph, co2airseaflux, flux_bury
