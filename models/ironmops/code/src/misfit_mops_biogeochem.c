...
/* Start dissolved Fe misfit */
/* Start dissolved Fe misfit */

/* Read vector of weights for individual tracer types and gridboxes */

	ierr = VecDuplicate(TR,&wbgc8);CHKERRQ(ierr);
	ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"dfe_weight.petsc",FILE_MODE_READ,&fd);CHKERRQ(ierr);
	ierr = VecLoad(wbgc8,fd);CHKERRQ(ierr);  /* IntoVector */ 
	ierr = PetscViewerDestroy(&fd);CHKERRQ(ierr);      
	
	ierr = PetscPrintf(PETSC_COMM_WORLD,"Read DFe misfit file for weighting\n");CHKERRQ(ierr);	

/* construct total weight: for now, multiply weight with fractional volume */

      ierr = VecDuplicate(TR,&w8);CHKERRQ(ierr);
      ierr = VecSet(w8,zero);CHKERRQ(ierr);
      ierr = VecPointwiseMult(w8,wbgc8,globVolFrac); /* IK So far, weigh these with volume fraction */
      ierr = PetscPrintf(PETSC_COMM_WORLD,"Set up weight arrays for DFe\n");CHKERRQ(ierr);	

/* IK : Check number of observations */

      ierr = VecSum(wbgc8,&NoSum8);CHKERRQ(ierr);
      ierr = PetscPrintf(PETSC_COMM_WORLD,"Number of observations: DFe: %12.0f \n",NoSum8);CHKERRQ(ierr);

/* IK : Compute global fractional volume of all locations where observations exist for weighting */

      ierr = VecSum(w8,&VolSum8);CHKERRQ(ierr);

/* Scale to get volume fraction of total observed volume */
             
      ierr = VecScale(w8,1.0/VolSum8);CHKERRQ(ierr);

/* Read the observations */ 

  ierr = VecDuplicate(TR,&obgc8);CHKERRQ(ierr);

  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"dfe_obs.petsc",FILE_MODE_READ,&fd);CHKERRQ(ierr);
  ierr = VecLoad(obgc8,fd);CHKERRQ(ierr);  /* IntoVector */ 
  ierr = PetscViewerDestroy(&fd);CHKERRQ(ierr);    
  ierr = PetscPrintf(PETSC_COMM_WORLD,"Read DFe observations\n");CHKERRQ(ierr);	

/* Initialize the cost function vectors */
	
  ierr = VecDuplicate(TR,&mbgc8avg);CHKERRQ(ierr);
  ierr = VecSet(mbgc8avg,zero);CHKERRQ(ierr);

  ierr = VecDuplicate(TR,&cbgc8);CHKERRQ(ierr);
  ierr = VecSet(cbgc8,zero);CHKERRQ(ierr);

/* End dissolved Fe misfit */
/* End dissolved Fe misfit */
...
/* Note: tc and iLoop are the time and step at the end of the current time step. */
  ...
  PetscInt ipo4=0,idop=1,ioxy=2,iphy=3,izoo=4,idet=5,ino3=6,idfe=7 ;  
...
//     if (misfitTimer->count<misfitTimer->numTimeSteps) {
	  ...
	  ierr = VecAXPY(mbgc7avg,one,state->c[idop]);CHKERRQ(ierr); /* DOP */
	  ierr = VecAXPY(mbgc8avg,one,state->c[idfe]);CHKERRQ(ierr); /* dFe*/
	  misfitTimer->count++;
// 	}
 }
...
/* IK : average model over a year; this will be stored again in mbgc1avg, ... */
		...
		ierr = VecScale(mbgc8avg,1.0/misfitTimer->count);CHKERRQ(ierr);
...
/* copy global mean tracer to array cbgc1, ... and perform subsequent operations on this */
		...
		ierr = VecCopy(mbgc8avg,cbgc8);CHKERRQ(ierr);
...
/* IK : Get the total cost: normalise by specific total volume and take square root */    
            ...
            Gorgcost = RGavecost4/Gaveobgc4+RGavecost5/Gaveobgc5+RGavecost6/Gaveobgc6+RGavecost7/Gaveobgc7;

/* IK End organic matter misfit */
/* IK End organic matter misfit */

/* IK Start dissolved Fe misfit */
/* IK Start dissolved Fe misfit */

/* IK : Calculate global average model tracers */

		ierr = VecDot(mbgc8avg,w8,&Gavembgc8);CHKERRQ(ierr);

/* IK : Calculate global average concentrations */

		ierr = VecDot(obgc8,w8,&Gaveobgc8);CHKERRQ(ierr); 

/* IK : subtract observations from average model concentrations; this will be stored in cbgc4avg, ....,  again */

		ierr = VecAXPY(cbgc8,minusone,obgc8);CHKERRQ(ierr);
		
/* IK : square the difference between observations and average model concentrations */

		ierr = VecPow(cbgc8,2.0);CHKERRQ(ierr);

/* IK : sum all local misfits, and scale with weight volume at the same time; result is scalar value Gavecost4, ... */

		ierr = VecDot(cbgc8,w8,&Gavecost8);CHKERRQ(ierr);

/* IK : Get the total cost: normalise by specific total volume and take square root */    

            RGavecost8=sqrt(Gavecost8);
            GDFecost = RGavecost8/Gaveobgc8;

/* IK End dissolved Fe misfit */
/* IK End dissolved Fe misfit */


/*IK Total RMSE Cost */ 

        /* RMSEMetric = Ginorgcost + Gorgcost + GDFecost; */
        RMSEMetric = Ginorgcost + GDFecost; 
...
/* VS Start Hellinger misfit */
            ...
            HDMetric = hd1 + hd2 + hd3 + hd4 + hd5 + hd6 + hd7;            
...
/* some diagnostic output for checking - only do this if not in transient cost function mode */
#ifndef TRANSIENTCOST
		ierr = PetscPrintf(PETSC_COMM_WORLD,"Writing  cost function at time %10.5f, step %d:: %10.5f %10.5f %10.5f %10.5f\n", tc, Iter0+iLoop,RMSEMetric,Ginorgcost,Gorgcost,GDFecost);CHKERRQ(ierr);    
		...
		ierr = PetscPrintf(PETSC_COMM_WORLD,"DFe components are DFe %10.5f \n", RGavecost8);CHKERRQ(ierr);                      
		ierr = PetscPrintf(PETSC_COMM_WORLD,"Average DFe observations are DFe %10.5f \n", Gaveobgc8);CHKERRQ(ierr);                      
		ierr = PetscPrintf(PETSC_COMM_WORLD,"Average DFe model tracers are DFe %10.5f\n", Gavembgc8);CHKERRQ(ierr);                      
		...
		ierr = PetscPrintf(PETSC_COMM_WORLD,"%10.5f %10.5f %10.5f %10.5f", Ginorgcost,Gorgcost,GDfecost,RMSEMetric);CHKERRQ(ierr); 
		ierr = PetscPrintf(PETSC_COMM_WORLD,"%10.5f %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f",RGavecost1,RGavecost2,RGavecost3,RGavecost4,RGavecost5,RGavecost6,RGavecost7,RGavecost8);
		ierr = PetscPrintf(PETSC_COMM_WORLD,"%10.5f %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f",Gavembgc1,Gavembgc2,Gavembgc3,Gavembgc4,Gavembgc5,Gavembgc6,Gavembgc7,Gavembgc8);CHKERRQ(ierr); 
		ierr = PetscPrintf(PETSC_COMM_WORLD,"%10.5f %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f",Gaveobgc1,Gaveobgc2,Gaveobgc3,Gaveobgc4,Gaveobgc5,Gaveobgc6,Gaveobgc7,Gaveobgc8);CHKERRQ(ierr); 
...
/* IK Start dissolved Fe misfit */
    ...
    ierr = VecDestroy(&mbgc8avg);CHKERRQ(ierr);
    ierr = VecDestroy(&obgc8);CHKERRQ(ierr);
    ierr = VecDestroy(&cbgc8);CHKERRQ(ierr);
