  ...
  for (ip=0; ip<lNumProfiles; ip++) {
    ...
    mops_biogeochem_ini_(&nzloc,&DeltaT,
			 ...
			 &localTs[kl],&localSs[kl],&localdz[kl],&drF[0],&nzmax,&nzeuph,
                         ...
                         if ((debug) & (ip==ipdebug)) {
PetscPrintf(PETSC_COMM_WORLD,"%g, %g, %g, %g, %g, %g, %g\n",localTs[kl],localSs[kl],localFes[kl],localswrad[ip],localtau[ip],localfice[ip],localwind[ip]);
PetscPrintf(PETSC_COMM_WORLD,"%g\n",ef->localph[ip]);
}
  }

/* Read and overwrite default parameter values here */
  ierr = PetscOptionsGetString(NULL,prefix,"-bgc_params_file",bgcParamsFile,PETSC_MAX_PATH_LEN-1,&ef->readBGCParams);CHKERRQ(ierr);

  if (ef->readBGCParams) {
    ierr = PetscOptionsGetInt(NULL,prefix,"-num_bgc_params",&numBGCParams,&flg);CHKERRQ(ierr);
...
PetscPrintf(PETSC_COMM_WORLD,"%g, %g\n",DICemp,ALKemp);
PetscPrintf(PETSC_COMM_WORLD,"%g, %g\n",localDIC[kl],localALK[kl]);
PetscPrintf(PETSC_COMM_WORLD,"%g, %g\n",localDIC[kl+nzloc-1],localALK[kl+nzloc-1]);
}
