...
PetscScalar dc13atm_ini = -6.5; /* default initial value */
PetscScalar dc13atm;
char dc13atmIniFile[PETSC_MAX_PATH_LEN];
PetscScalar pC13O2atm; /* This is the prognostic (time-stepped) variable: its initial value is calculated below */
...
#if defined O_carbon_13_coupled
/* overwrite default value */
  ierr = PetscOptionsGetReal(PETSC_NULL,"-dc13atm_ini",&dc13atm_ini,&flg);CHKERRQ(ierr); /* read from command line */
  if (!flg) {
	...
	if (flg) { /* read from binary file */
	  ...
	  ierr = PetscViewerBinaryGetDescriptor(fd,&fp);CHKERRQ(ierr);
	  ierr = PetscBinaryRead(fp,&dc13atm_ini,1,PETSC_SCALAR);CHKERRQ(ierr);
	  ierr = PetscViewerDestroy(&fd);CHKERRQ(ierr);
	}
  }
  dc13atm = dc13atm_ini;
...
/*    EmPglobavg = 0.0; */ /* this is set to zero above */
  ...
  if (calcDiagnostics) {  
	if (Iter0+iLoop==diagTimer.startTimeStep) { /* start time averaging (note: diagStartTimeStep is ABSOLUTE time step) */	
	  mobi_diags_start_(&debugFlag); /* NOTE: we only call this once */
	}	
  }                         
...
/*  The virtual flux term included in localtotflux should go be zero when integrated over the global ocean surface */
  for (ip=0; ip<lNumProfiles; ip++) {
	localFocean = localFocean + localgasexflux[ip]*localdA[ip]*(12.0/1.e15)*secondsPerYear; /* PgC/y */
#if defined O_carbon_13_coupled
/* SPK: we use the same mass as C12 to convert */
	localF13ocean = localF13ocean + localc13gasexflux[ip]*localdA[ip]*(12.0/1.e15)*secondsPerYear; /* PgC/y */
#endif
  }
#endif
#endif /* O_carbon */
