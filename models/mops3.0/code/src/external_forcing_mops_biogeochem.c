...
// Common data (only initialize/read once)
  ...
  // Now set data for this instance
...
/* Read and overwrite default parameter values here */
  ierr = PetscOptionsGetString(NULL,prefix,"-bgc_params_file",ef->bgcParamsFile,PETSC_MAX_PATH_LEN-1,&ef->readBGCParams);CHKERRQ(ierr);

  if (ef->readBGCParams) {
    ierr = PetscOptionsGetInt(NULL,prefix,"-num_bgc_params",&ef->numBGCParams,&flg);CHKERRQ(ierr);
    if (!flg) SETERRQ(PETSC_COMM_WORLD,1,"Must indicate number of BGC parameters to read with the -num_bgc_params option");    

    ierr = PetscPrintf(PETSC_COMM_WORLD,"Reading %d parameters from file\n",ef->numBGCParams);CHKERRQ(ierr);

#ifdef ASCIIPARAMS
    fpparams=fopen(ef->bgcParamsFile,"r");
    for(ipar=0;ipar<ef->numBGCParams;ipar++) {
      ierr = fscanf(fpparams,"%lf\n",&bgcparams[ipar]);
    }   
    fclose(fpparams);
#else
    ierr = PetscViewerBinaryOpen(PETSC_COMM_SELF,ef->bgcParamsFile,FILE_MODE_READ,&fd);CHKERRQ(ierr);
    ierr = PetscViewerBinaryGetDescriptor(fd,&fp);CHKERRQ(ierr);
    ierr = PetscMalloc(ef->numBGCParams*sizeof(PetscScalar),&bgcparams);CHKERRQ(ierr);
    ierr = PetscBinaryRead(fp,bgcparams,ef->numBGCParams,NULL,PETSC_SCALAR);CHKERRQ(ierr); 
    ierr = PetscViewerDestroy(&fd);CHKERRQ(ierr);
#endif
    ...
    mops_biogeochem_set_params_(&ef->numBGCParams,&bgcparams[0]);
...
/*Data for diagnostics */
	...
	ierr = VecDuplicate(TR,&ef->fbgc8);CHKERRQ(ierr);
	ierr = VecSet(ef->fbgc8,zero);CHKERRQ(ierr);
	ierr = VecGetArray(ef->fbgc8,&ef->localfbgc8);CHKERRQ(ierr);
	ierr = VecDuplicate(TR,&ef->fbgc8avg);CHKERRQ(ierr);
	ierr = VecSet(ef->fbgc8avg,zero);CHKERRQ(ierr);
	ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,diagOutFile[7],FILE_MODE_WRITE,&ef->fdfbgc8avg);CHKERRQ(ierr);

	ierr = VecDuplicate(TR,&ef->fbgc9);CHKERRQ(ierr);
	ierr = VecSet(ef->fbgc9,zero);CHKERRQ(ierr);
	ierr = VecGetArray(ef->fbgc9,&ef->localfbgc9);CHKERRQ(ierr);
	ierr = VecDuplicate(TR,&ef->fbgc9avg);CHKERRQ(ierr);
...
/* accumulate/write instantaneous atmos model state */
	   ...
	   if (Iter0+iLoop>=(ef->atmWriteTimer->startTimeStep)) { /* note: startTimeStep is ABSOLUTE time step */
	     if ((ef->atmWriteTimer->count)==(ef->atmWriteTimer->numTimeSteps) || (Iter0+iLoop==ef->atmWriteTimer->startTimeStep)) { /* time to write out */
  ...
  if (ef->calcDiagnostics) {  
      ...
      for (ip=0; ip<lNumProfiles; ip++) {
        ...
        ierr = VecScale(ef->fbgc8avg,1.0/ef->diagTimer->count);CHKERRQ(ierr);
        ierr = VecView(ef->fbgc8avg,ef->fdfbgc8avg);CHKERRQ(ierr);
        ierr = VecSet(ef->fbgc8avg,zero); CHKERRQ(ierr);
...
/* write instantaneous atmos model state */
  ...
  if (ef->calcDiagnostics) {  
	...
	ierr = VecDestroy(&ef->fbgc8);CHKERRQ(ierr);
	ierr = VecDestroy(&ef->fbgc8avg);CHKERRQ(ierr);
    ierr = PetscViewerDestroy(&ef->fdfbgc8avg);CHKERRQ(ierr);	
...
PetscErrorCode reInitializeExternalForcing(PetscScalar tc, PetscInt Iter, PetscInt iLoop, TMMState state, void *userctx)
{
  ...
  PetscInt ipar;
  PetscBool flg;
  char runoffReinitializeFile[PETSC_MAX_PATH_LEN];
  PetscScalar runoff_ini = 0.0;  
...
//CIKADDEDBLOCK
/* set inititial runoff to value set in iniExternalForcing */
/* set inititial runoff: overwrite default value with value from command line*/
/* set inititial runoff: overwrite default value with value from file*/
  ierr = PetscOptionsGetString(NULL,prefix,"-runoff_reinitialize_file",runoffReinitializeFile,PETSC_MAX_PATH_LEN-1,&flg);CHKERRQ(ierr);
  if (flg) { /* read from binary file */
	ierr = PetscViewerBinaryOpen(PETSC_COMM_SELF,runoffReinitializeFile,FILE_MODE_READ,&fd);CHKERRQ(ierr);
	ierr = PetscViewerBinaryGetDescriptor(fd,&fp);CHKERRQ(ierr);
	ierr = PetscBinaryRead(fp,&runoff_ini,1,NULL,PETSC_SCALAR);CHKERRQ(ierr);
	ierr = PetscViewerDestroy(&fd);CHKERRQ(ierr);
  }
  ...
  ef->GRunoff = runoff_ini;
  ierr = PetscPrintf(PETSC_COMM_WORLD,"Reinitializing runoff to %g Gmol P/d\n",ef->GRunoff);CHKERRQ(ierr);
//CIKADDEDBLOCK
